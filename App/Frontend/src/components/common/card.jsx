...
const Card = ({ title, value, subtitle, bgColor, iconPath, iconPath2, size = 'large', className }) => {
    ...
    return (
        <div className={`w-full flex ${user.livello !== 0 ? 'flex-col' : 'items-center justify-between'} bg-${bgColor} bg-opacity-40 rounded-[30px] ${currentSize.padding} h-auto ${className || ''}`}>
            {user.livello !== 0 && (
            ...
            {user.livello !== 0 ? (
                <div className={`flex justify-center h-full justify-between h-full items-center space-x-[1rem]`}>
