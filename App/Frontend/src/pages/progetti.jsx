...
const Progetti = () => {
  ...
  const fetchProgetti = async () => {
    try {
      ...
      // Livello studente: progetto fisso
      if (user.livello == 1) {
        const response = await progettiService.getByCodice(user.codiceprogetto);
      ...
      if (user.livello >= 2) {
        ...
        if (codiciProgetti.length > 0) {
          const responses = await Promise.all(
            codiciProgetti.map(codice => progettiService.getByCodice(codice))
          );
          const progettiData = responses.flatMap(r =>
            Array.isArray(r) ? r :
              r?.data && Array.isArray(r.data) ? r.data :
                r?.progetti && Array.isArray(r.progetti) ? r.progetti : []
          );
          console.log('Progetti recuperati per docente:', progettiData);
          setProgetti(progettiData);
        }
        return;
      }

      // Livello admin (0): tutti i progetti
  ...
  useEffect(() => {
    fetchProgetti();
    fetchAnni();
    if (user.livello === 0 || user.livello === 3) {
      fetchDocenti();
    }
  }, []);
              ...
              <Container className="flex flex-col h-full overflow-hidden p-[1vw]">
                <div className="flex gap-[1vw] items-end">
                  <Select
                    ...
                    className="w-[40%]"
                    disable={user.livello === 1 || progetti.length == 0}
                  />
                  <Select
                    title="Anno"
                    ...
                    className="w-[20%]"
                    disable={user.livello === 1}
                  />
                  {((selectedAnno || selectedProgetto) && progetti.length > 0) ?
