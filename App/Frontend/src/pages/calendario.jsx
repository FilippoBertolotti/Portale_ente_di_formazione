...
import { isCFValid, isOraInizioValid, isOraFineValid, isModuleValid, isClassroomValid, isDateValid2, isPlaceNameValid, isDescriptionValid, isDateValid, isCourseValid } from '../utils/validators';
  ...
  const fetchLezioni = async () => {
    try {
      ...
      if (user.livello == 0 || user.livello == 4) {
        ...
        const progettidata = await progettiService.getAll();
        setProgetti(progettidata.data || []);
      } else if (user.livello >= 2) {
        // Unisci i codici progetto (docente + coordinatore) senza duplicati
        const codiciDocente = Array.isArray(user.codiceprogetto)
          ? user.codiceprogetto
          : user.codiceprogetto ? [user.codiceprogetto] : [];

        const codiciCoordinatore = Array.isArray(user.codicecoordinatore)
          ? user.codicecoordinatore
          : user.codicecoordinatore ? [user.codicecoordinatore] : [];

        // Set per eliminare duplicati automaticamente
        const tuttiICodici = [...new Set([...codiciDocente, ...codiciCoordinatore])];

        if (tuttiICodici.length > 0) {
          const responses = await Promise.all(
            tuttiICodici.map(codice => lezioniService.getByProgetto(codice))
          );
          const data = responses.flatMap(r =>
            Array.isArray(r) ? r :
              r?.data && Array.isArray(r.data) ? r.data :
                r?.lezioni && Array.isArray(r.lezioni) ? r.lezioni : []
          );

          // Deduplicazione per id lezione (nel caso lo stesso progetto fosse in entrambe le liste)
          const lezioniUniche = data.filter((l, index, self) =>
            index === self.findIndex(x => x.id === l.id)
          );
          setLezioni(lezioniUniche);
        }

        // Moduli filtrati: tutti quelli dei progetti accessibili
        const modulidata = await Promise.all(
            codiciCoordinatore.map(codice => moduliService.getByCodiceProgetto(codice))
        );
        setModuli(modulidata.data || modulidata.flatMap(r => r.data || []));

        const progettidata = await Promise.all(
            codiciCoordinatore.map(codice => progettiService.getByCodice(codice))
        );
        setProgetti(progettidata.data || progettidata.flatMap(r => r.data || []));
      } else {
        const data = await lezioniService.getByProgetto(user.codiceprogetto);
        setLezioni(data.data || []);
      }
    ...
    <div className="flex flex-col h-full w-full">
          ...
          <Container
            title={
              <div className="flex items-center gap-[1rem]">
                ...
                />}></Button>
                <span className="font-bold text-[1.2rem] capitalize w-[9vw] text-center">{meseAnno}</span>
            ...
            button={
              <div className="flex items-center gap-[1rem]">
                ...
                <Button variant="tertiary" size="medium"
                  ...
                  className={`${user.livello !== 0 && user.livello !== 3 ? 'hidden' : ''}`}
                >
                  Nuova Lezione
                </Button>
              </div>
      ...
      >
        <Form
          ...
          placeholders={[selectedDate, null, null, 'Seleziona un modulo', 'Seleziona un aula', 'Seleziona un progetto']}
          validators={[isDateValid2, isOraInizioValid, isOraFineValid, isModuleValid, isClassroomValid, isCourseValid]}
          options={{
            codiceprogetto: Array.isArray(progetti) ? progetti.map(p => ({ value: p.codice, label: p.descrizione })) : [],
      ...
      >
        <Form
          ...
          validators={[null, isOraInizioValid, isOraFineValid, isModuleValid, isClassroomValid, isCourseValid]}
          options={{
            codiceprogetto: Array.isArray(progetti) ? progetti.map(p => ({ value: p.codice, label: p.descrizione })) : [],
            idmodulo: Array.isArray(moduli) ? moduli.map(m => ({ value: m.id, label: m.descrizione })) : [],
            idaula: Array.isArray(aule) ? aule.map(a => ({ value: a.id, label: a.descrizione })) : []
          ...
          defaultValues={{
            ...
            codiceprogetto: progetti.find(p => p.codice === selectedLezione?.codice)?.codice || ''
          }}
        />
      </ConfirmationModal>
