#Formazione Mirata
##Installazione e configurazione
Per una corretta installazione, fino a quando il servizio non verrà hostato, in seguito a scaricare la repository sarà necessario avere una propria chiave per l’API e un proprio database da collegare sia a backend che a frontend, dato che i file .env non sono stati inseriti nella repository.
In seguito una volta che si avrà tutte le informazioni necessarie bisognerà entrare avviare sia il backend che il frontend entrando nelle cartelle corrispondenti tramite cmd, verificare la corretta installazione dei node_modules con il comando 
'npm i' 
ed infine avviare il tutto con il comando
'npm run dev'
